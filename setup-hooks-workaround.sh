...
STABLE_LINK="$HOME/.claude/hooks/terminal-visual-signals-current"
PLUGIN_BASE="$HOME/.claude/plugins/cache/terminal-visual-signals/terminal-visual-signals"
...
"hooks": {
  "Notification": [
    {
      ...
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh permission" }]
    },
    {
      "matcher": "idle_prompt",
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh idle" }]
    }
  ],
  "PermissionRequest": [
    {
  ...
  "PostToolUse": [
    {
      "matcher": "*",
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh processing" }]
    }
  ],
  "PreCompact": [
    {
      "matcher": "auto",
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh compacting" }]
    },
    {
      "matcher": "manual",
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh compacting" }]
  ...
  "SessionEnd": [
    {
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh reset" }]
    }
  ],
  "SessionStart": [
    {
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-session-start.sh" }]
    }
  ],
  "Stop": [
    {
      "hooks": [{ "type": "command", "command": "bash ~/.claude/hooks/terminal-visual-signals-current/scripts/claude-code-visual-signal.sh complete" }]
    }
  ],
  "UserPromptSubmit": [
    {
...
create_session_start_script() {
    cat > "$HOOKS_DIR/terminal-visual-signals-session-start.sh" << 'SESSION_EOF'
#!/usr/bin/env bash
...
SESSION_EOF
    chmod +x "$HOOKS_DIR/terminal-visual-signals-session-start.sh"
}

do_install() {
    echo "Setting up terminal-visual-signals workaround..."
    echo ""
    ...
    if [[ ! -d "$PLUGIN_BASE" ]]; then
        ...
        echo "  claude plugin install terminal-visual-signals@terminal-visual-signals"
        exit 1
    fi
    ...
    ln -s "$VERSION_DIR" "$STABLE_LINK"
    echo "✓ Created symlink: terminal-visual-signals-current → $VERSION"

    # Create session start script (auto-updates symlink on version change)
    create_session_start_script
    echo "✓ Created auto-updater: terminal-visual-signals-session-start.sh"
...
do_uninstall() {
    echo "Removing terminal-visual-signals workaround..."
    echo ""

    # Remove symlink
    if [[ -L "$STABLE_LINK" ]]; then
        rm -f "$STABLE_LINK"
        echo "✓ Removed symlink: terminal-visual-signals-current"
    else
        echo "✓ Symlink already removed"
    fi

    # Remove session start script
    if [[ -f "$HOOKS_DIR/terminal-visual-signals-session-start.sh" ]]; then
        rm -f "$HOOKS_DIR/terminal-visual-signals-session-start.sh"
        echo "✓ Removed auto-updater: terminal-visual-signals-session-start.sh"
    else
    ...
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "Remove all hooks containing 'terminal-visual-signals' from:"
