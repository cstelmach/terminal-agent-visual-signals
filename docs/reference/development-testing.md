...
```bash
# Copy all core files from repo to plugin cache (auto-detect version)
...
2. Sync cache: `./tavs sync` (or use the one-liner above)
...
1. **Verify cache was updated:**
   ```bash
   ls -la ~/.claude/plugins/cache/terminal-agent-visual-signals/tavs/*/src/core/trigger.sh
